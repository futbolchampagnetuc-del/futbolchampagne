-- ============================================
-- FutbolChampagne — Migración Inicial
-- Versión: 001
-- Fecha: 2026-07-22
-- ============================================
...
-- Auto-crear perfil cuando se registra un usuario con Google
...
create index idx_asignacion_partido on asignacion_equipos(partido_id);
...
create index idx_goles_partido on goles_partido(partido_id);
create index idx_goles_jugador on goles_partido(jugador_id);
...
create table if not exists evaluaciones (
  ...
  check (evaluador_id != evaluado_id) -- No autovoto
);

create index idx_evaluaciones_partido on evaluaciones(partido_id);
...
-- Permisos: lectura para autenticados
grant select on rankings to authenticated;

-- Trigger para refrescar rankings después de insert/update en evaluaciones o goles
